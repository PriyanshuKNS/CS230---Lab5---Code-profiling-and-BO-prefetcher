...
// CONFIGURATION
// VARIABLE PARAMETER
// #define ITER 1                  // Maximum number of iterations
...
char* DeCrypt(char *ciphertext, char *pswd)
{
    ...
    int lenCiphertext = my_strlen(ciphertext);

    char* buff = (char*)malloc(lenCiphertext+1);
    memcpy(buff, ciphertext, lenCiphertext+1);

    // int px = 0, py = 0;
    
    int plimit = PASS_X_LIM * PASS_Y_LIM;
    int p = 0;
    int pmod = 0;

    int prf_int = (key[p].li ^ key[p].zi ^ key[p].ki) & (15);


    int mask = key[p].a[(prf_int+1) >> 4];
    ...
    do{
        ...
        prf_int = (key[p].li ^ key[p].zi ^ key[p].ki) & (15);
...
{
    int lenPlaintext = my_strlen(plaintext);
    ...
    if ((CalculateCRC(plaintext) == validCRC) && (!my_memcmp(plaintext, correct_plaintext, lenPlaintext)))
        return 1;
    return 0;
}
...
void do_pswd(char *ciphertext, char *pswd, int validCRC, int progress)
{
    ...
    char* temp = DeCrypt(ciphertext, pswd);
    int len = my_strlen(temp);
    char* extra = (char*)malloc(len+1);
    memcpy(extra, temp, len+1);

    // Checking the password CRC
    if (CheckCRC(temp, validCRC))
    { 
    // CRC match
    // Decrypting
    buff = extra;
